...
> **FORK NOTE**: This is a local fork of [hexo-theme-fluid v1.9.9](https://github.com/fluid-dev/hexo-theme-fluid) for the BillZong.github.io blog.
> Only `layout/_partials/plugins/mermaid.ejs` is modified to support mermaid 11.x (`mermaid.run()` + fallback guard).
> Hexo loads `themes/fluid/` with higher priority than `node_modules/hexo-theme-fluid/`, so this fork is the effective source.
> To upgrade: re-copy the upstream `layout/`, `source/`, `scripts/`, `languages/` directories and re-apply the mermaid patch.
